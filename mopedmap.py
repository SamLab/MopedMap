...
REGION_ALIASES = [
    ...
    {"pattern": "климовское", "name": "Климовское", "lat": 57.51444, "lon": 39.88778, "type": "city", "subject": "Ярославская область"},
    ...
    {"pattern": "мелитополю", "name": "Мелитополь", "lat": 46.842, "lon": 35.365, "type": "city", "subject": "Запорожская область"},
    {"pattern": "мелитополем", "name": "Мелитополь", "lat": 46.842, "lon": 35.365, "type": "city", "subject": "Запорожская область"},
    {"pattern": "мелитополе", "name": "Мелитополь", "lat": 46.842, "lon": 35.365, "type": "city", "subject": "Запорожская область"},
    # Мариуполь, Донецкая область (Украина) — город в cities.json, падежи на -ь здесь явно
    {"pattern": "мариуполя", "name": "Мариуполь", "lat": 47.09712, "lon": 37.54383, "type": "city", "subject": "Донецкая область"},
    {"pattern": "мариуполю", "name": "Мариуполь", "lat": 47.09712, "lon": 37.54383, "type": "city", "subject": "Донецкая область"},
    {"pattern": "мариуполем", "name": "Мариуполь", "lat": 47.09712, "lon": 37.54383, "type": "city", "subject": "Донецкая область"},
    {"pattern": "мариуполе", "name": "Мариуполь", "lat": 47.09712, "lon": 37.54383, "type": "city", "subject": "Донецкая область"},
...
DISAMBIGUATION_MAP = {
    ...
    "красногорский": {
        ...
        "московская область": [
            {"context_subject": "брянская область", "lat": 53.0, "lon": 31.6, "name": "Красная Гора", "subject": "Брянская область"},
            {"context_subject": "удмуртия", "lat": 57.70694, "lon": 52.49694, "name": "Красногорское", "subject": "Удмуртия"},
        ],
    },
    "монастырщинский": {
    ...
    "октябрьский": {
        "костромская область": [
            ...
            {"context_subject": "курская область", "lat": 51.65, "lon": 35.933, "name": "Прямицыно", "subject": "Курская область"},
        ],
        "__any__": [
            {"context_subject": "белгородская область", "lat": 50.44, "lon": 36.35, "name": "Октябрьский", "subject": "Белгородская область"},
            {"context_subject": "республика крым", "lat": 45.35, "lon": 36.48, "name": "Октябрьский", "subject": "Республика Крым"},
    ...
    "мантуровский": {
        ...
        "курская область": [
            {
                "context_subject": "костромская область",
                "lat": 58.33, "lon": 44.76,
                "name": "Мантурово",
    ...
    "краснинский": {
        "липецкая область": [
            {"context_subject": "смоленская область", "lat": 54.55, "lon": 31.433, "name": "Красный", "subject": "Смоленская область"},
        ],
        "смоленская область": [
            {"context_subject": "липецкая область", "lat": 52.85, "lon": 38.783, "name": "Красное", "subject": "Липецкая область"},
        ],
    },
    "дмитровский": {
        "московская область": [
            {"context_subject": "орловская область", "lat": 52.5, "lon": 35.15, "name": "Дмитровск", "subject": "Орловская область"},
        ],
        "орловская область": [
            {"context_subject": "московская область", "lat": 56.35, "lon": 37.52, "name": "Дмитров", "subject": "Московская область"},
        ],
    },
    "каменский": {
        "воронежская область": [
            {"context_subject": "тульская область", "lat": 53.35, "lon": 37.667, "name": "Архангельское", "subject": "Тульская область"},
        ],
        "тульская область": [
            {"context_subject": "воронежская область", "lat": 50.717, "lon": 39.417, "name": "Каменка", "subject": "Воронежская область"},
        ],
    },
    "кашира": {
        "московская область": {
    ...
    for r in results:
        ...
        for suffix in (" район", " р-н", " районе", " р-не"):
            if matched_key.endswith(suffix):
                ...
                break
        # Именные ключи в DISAMBIGUATION_MAP чаще описаны в именительном падеже
        # («октябрьский», «жуковский»), а в постах явно звучит предложный
        # («в Октябрьском районе», «Жуковском районе»). Пробуем оба варианта:
        # «...ском/цком/ского/цкого» → «...ский/цкий».
        def _nom_key(k):
            for suf, rep in (("ского", "ский"), ("цкого", "цкий"), ("ском", "ский"), ("цком", "цкий")):
                if k.endswith(suf):
                    return k[:-len(suf)] + rep
            return k
        keys = []
        for _k in (nl, matched_key):
            keys.append(_k)
            _nk = _nom_key(_k)
            if _nk != _k:
                keys.append(_nk)
...
def build_region_feed(posts_data, max_items=20):
    """Собирает ленту постов по Ярославской области и 5 соседним регионам.
...
SUMMARY_PATTERNS = [
    ...
    r'в течение дня.*дежурными (?:силами|средствами) пво',
    r'в период с.*дежурными (?:силами|средствами) пво',
...
NIGHT_KILLS_RE = re.compile(
    r'за прошедш\w+ ноч\w*'
    r'[^.]{0,80}?'
    r'(?:пво|противовоздушн\w+ обороны)'
    r'[^.]{0,80}?'
    r'(?:было\s+)?'
...
DAY_KILLS_RE = re.compile(
    r'(?:в течение дня'
    r'|с \d{1,2}[:.]\d{2}\s*(?:мск\s+)?до \d{1,2}[:.]\d{2}\s*(?:мск\s*)?)'
    r'.{0,120}?'
    r'(?:дежурными\s+)?(?:силами|средствами)\s+(?:пво|противовоздушн\w+ обороны)'
    r'.{0,120}?'
    r'(?:было\s+)?'
    r'(?:уничтожен\w*|сбит\w*|перехвачен\w*)'
    r'(?:\s+и\s+(?:уничтожен\w*|сбит\w*|перехвачен\w*))?'
...
def _extract_recap_number(posts, regex):
    ...
    for post_item in posts or []:
        if not isinstance(post_item, tuple) or len(post_item) < 3:
            continue
        if post_item[2] != 'locatorru':
            continue
        text = post_item[0]
        if not text:
            continue
